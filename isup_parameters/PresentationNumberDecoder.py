...
from isup_parameters.PresentationNumberParameter import *
...
class PresentationNumberDecoder:
    ...
    def decode(stream):
        ...
        if (stream.readField(1)):
            flags.add(PresentationNumberFlag.OddAddressSignals)
        ...
        if (stream.readField(1)):
            flags.add(PresentationNumberFlag.PresentationNumberPreferred)


        addressSignals = []
        while(stream.hasMore()):
            addressSignalsParameter = AddressSignalsDecoder.decode(stream)
            # PresentationNumberFlag::OddAddressSignals is TRUE if there are an ODD number of addressSignals
            # (We also need to skip any padding in this case)
            if (stream.hasMore() or (PresentationNumberFlag.OddAddressSignals not in flags)):
                addressSignals.append(addressSignalsParameter)
        return PresentationNumberParameter(flags,
